...
const BuyCourseBtn = ({ current_user, course }) => {
  ...
  return alreadyBuy ? (
    <button className="default-btn" onClick={() => router.push("/checkout")}>
      <i className="flaticon-user"></i> Afficher mes cours <span></span>
    </button>
  ) : (
    <>
      {add ? (
        ...
        <button className="default-btn" onClick={() => buyCourse(course)}>
          <i className="flaticon-shopping-cart"></i> Ajouter ce Cours
          <span></span>
        </button>
      )}
    </>
